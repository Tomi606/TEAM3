DROP DATABASE IF EXISTS `rudgh0307`;

CREATE DATABASE IF NOT EXISTS `rudgh0307`;
...
ADD CONSTRAINT `FK_member_TO_bookmark_1`
  ...
  REFERENCES `rudgh0307`.`member` (`me_id`)
  ON DELETE CASCADE
  ON UPDATE CASCADE;

#댓글 cascade
ALTER TABLE `rudgh0307`.`comment` 
DROP FOREIGN KEY `FK_post_TO_comment_1`;
ALTER TABLE `rudgh0307`.`comment` 
ADD CONSTRAINT `FK_post_TO_comment_1`
  FOREIGN KEY (`co_po_num`)
  REFERENCES `rudgh0307`.`post` (`po_num`)
  ON DELETE CASCADE
  ON UPDATE CASCADE;
...
ADD CONSTRAINT `FK_site_management_TO_comment_1`
  ...
  REFERENCES `rudgh0307`.`site_management` (`site_num`)
  ON DELETE CASCADE
  ON UPDATE CASCADE;

#병원

ALTER TABLE `rudgh0307`.`hospital` 
DROP FOREIGN KEY `FK_hospital_subject_TO_hospital_1`;
ALTER TABLE `rudgh0307`.`hospital` 
ADD CONSTRAINT `FK_hospital_subject_TO_hospital_1`
  FOREIGN KEY (`ho_hs_num`)
  REFERENCES `rudgh0307`.`hospital_subject` (`hs_num`)
  ON DELETE CASCADE
  ON UPDATE CASCADE;
...
ADD CONSTRAINT `FK_land_TO_hospital_1`
  ...
  REFERENCES `rudgh0307`.`land` (`la_num`)
  ON DELETE CASCADE
  ON UPDATE CASCADE;
...
ADD CONSTRAINT `FK_member_state_TO_hospital_1`
  ...
  REFERENCES `rudgh0307`.`member_state` (`ms_state`)
  ON DELETE CASCADE
  ON UPDATE CASCADE;

#상세

ALTER TABLE `rudgh0307`.`hospital_detail` 
...
ALTER TABLE `rudgh0307`.`hospital_program` 
...
ADD CONSTRAINT `FK_hs_list_TO_hospital_program_1`
  ...
  REFERENCES `rudgh0307`.`hs_list` (`hsl_num`)
  ON DELETE CASCADE
  ON UPDATE CASCADE;

#과목리스트

ALTER TABLE `rudgh0307`.`hs_list` 
...
ALTER TABLE `rudgh0307`.`item_list` 
...
ADD CONSTRAINT `FK_hospital_program_TO_item_list_1`
  ...
  REFERENCES `rudgh0307`.`hospital_program` (`hp_num`)
  ON DELETE CASCADE
  ON UPDATE CASCADE,
ADD CONSTRAINT `FK_item_TO_item_list_1`
  FOREIGN KEY (`il_it_num`)
  REFERENCES `rudgh0307`.`item` (`it_num`)
  ON DELETE CASCADE
  ON UPDATE CASCADE;

#멤버

ALTER TABLE `rudgh0307`.`member` 
...
ADD CONSTRAINT `FK_payment_state_TO_payment_1`
  ...
  REFERENCES `rudgh0307`.`payment_state` (`ps_name`)
  ON DELETE CASCADE
  ON UPDATE CASCADE,
ADD CONSTRAINT `FK_reservation_TO_payment_1`
  FOREIGN KEY (`pm_rv_num`)
  REFERENCES `rudgh0307`.`reservation` (`rv_num`)
  ON DELETE CASCADE
  ON UPDATE CASCADE;

#포스트
ALTER TABLE `rudgh0307`.`post` 
...
ADD CONSTRAINT `FK_board_TO_post_1`
  ...
  REFERENCES `rudgh0307`.`board` (`bo_num`)
  ON DELETE CASCADE
  ON UPDATE CASCADE,
...
ALTER TABLE `rudgh0307`.`recommend` 
...
ADD CONSTRAINT `FK_report_state_TO_report_1`
  ...
  REFERENCES `rudgh0307`.`report_state` (`rs_name`)
  ON DELETE CASCADE
  ON UPDATE CASCADE,
...
ALTER TABLE `rudgh0307`.`reservation` 
...
ADD CONSTRAINT `FK_reservation_schedule_TO_reservation_1`
  ...
  REFERENCES `rudgh0307`.`reservation_schedule` (`rs_num`)
  ON DELETE CASCADE
  ON UPDATE CASCADE,
ADD CONSTRAINT `FK_reservation_state_TO_reservation_1`
  FOREIGN KEY (`rv_rvs_name`)
  REFERENCES `rudgh0307`.`reservation_state` (`rvs_name`)
  ON DELETE CASCADE
  ON UPDATE CASCADE;

#리저베이션 스케쥴

ALTER TABLE `rudgh0307`.`reservation_schedule` 
...
ADD CONSTRAINT `FK_hospital_detail_TO_review_1`
  ...
  REFERENCES `rudgh0307`.`hospital_detail` (`hd_num`)
  ON DELETE CASCADE
  ON UPDATE CASCADE,
...
ALTER TABLE `rudgh0307`.`site_management` 
...
#결제상태
INSERT INTO `rudgh0307`.`payment_state` (`ps_name`) VALUES ('결제완료');
INSERT INTO `rudgh0307`.`payment_state` (`ps_name`) VALUES ('결제취소');
